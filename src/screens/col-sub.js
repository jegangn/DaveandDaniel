// Daniel · OP: GETAWAY — N-digit column subtraction.
// Before the kid answers, the borrow pre-pass auto-animates the regrouping
// (strike the lent digit, write its new value above), chaining through zeros.
// The kid then reads the regrouped numbers and drops the answer digits R-to-L.
import { createDragManager } from "../drag.js";
import { tilePickup, tileBounceBack, tileSnapIn, animateBorrowChain } from "../animate.js";
import { isComplete } from "../logic.js";
import {
  getProblemsDaniel, mulberry32, analyzeColumnsSub, createAnswerStateN, dropDigit,
...
export function mount(stage, ctx, router) {
  ...
  const problems = getProblemsDaniel(world, level, mulberry32((Date.now() ^ (level * 40503)) >>> 0));
  let idx = 0;
  let totalWrong = 0;
  ...
  renderProgressDots();
  renderTray();
  // Attach BEFORE renderProblem — the borrow pre-pass measures cells in the DOM.
  ...
  async function renderProblem() {
    ...
    const a = analyzeColumnsSub(p.a, p.b);
    activeState = createAnswerStateN(p.answer);
    const N = a.width;
    const aTop = padLeft(a.aDigits, N);
    const bBot = padLeft(a.bDigits, N);

    // The difference can be shorter than the operands (e.g. 312−298=14): render
    // answer slots right-aligned, leaving the leading columns blank.
    const ansLen = String(p.answer).length;
    const offset = N - ansLen;

    const ws = sec.querySelector(".col-ws");
    ws.style.gridTemplateColumns = `52px repeat(${N}, var(--col-w))`;
    let mark = "", top = "", bot = "", ans = "";
    for (let c = 0; c < N; c++) {
      const rtl = (N - 1) - c; // RTL column index for borrow-step lookup
      mark += `<div class="carry-cell" data-col="${c}"></div>`;
      top += `<div class="cell" data-rtl="${rtl}">${aTop[c]}</div>`;
      bot += `<div class="cell">${bBot[c]}</div>`;
      if (c < offset) {
        ans += `<div class="cell"></div>`;
      } else {
        const di = c - offset; // answer-digit index (0 = most significant)
        ans += `<div class="slot ${di === ansLen - 1 ? "active" : "inactive"}" data-index="${di}"></div>`;
      }
    }
    ws.innerHTML = `
      <div class="ws-op"></div>${mark}
    ...
    // Auto-borrow pre-pass (kid watches; never performs it).
    await animateBorrowChain(sec, a.steps);
  }
